...
const vehiclesSelect = props.vehicles.map(x => ({ value: x.id, name: x.nombre }))
</script>
    ...
    <DefaultLayout>
        <div class="py-8 px-4 mx-auto max-w-screen-lg text-center lg:py-16">
            ...
            <form @submit.prevent="form.post(route('channels.store'))" class="max-w-lg mx-auto text-left">
                <div class="mb-5">
                    <label for="handle" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Vehiculo</label>
                    <FormError :error="form.vehicl?.handle" />
